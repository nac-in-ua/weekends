...
test.describe('Settings modal', () => {
  ...
  test('Input fields validation', async ({ page }) => {
    ...
    await test.step('should highlight greetings input with red color', async () => {
      const input = page.locator('[placeholder="Have a beer"]')
      await input.fill('')
    ...
    await test.step('should not allow to type more than 20 characters', async () => {
      const input = page.locator('[placeholder="Have a beer"]')
  ...
  test('Verify settings modal behaviour', async ({ page }) => {
    ...
    await test.step('should open settings modal with previous values', async () => {
      await page.locator('svg').first().click()
      const modal = page.locator('[data-testid="settings-modal"]')
      expect(modal).toBeVisible()
  ...
  test('Verify that application can use system theme', async ({ page }) => {
    ...
    await test.step('should adjust app theme not to use system theme', async () => {
      await page.locator('button:has-text("Apply")').click()
